...
import "./WormholeGetters.sol";
import "./WormholeMessages.sol";

/**
 * @title A Cross-Chain Wormhole Application
 * @notice This contract uses Wormhole's generic-messaging to send an arbitrary
 * Wormhole message to registered emitters on foreign blockchains
 */
contract Wormhole is WormholeGetters, WormholeMessages {
     ...
     * See WormholeState.sol for descriptions of each state variable.
     */
    ...
    /**
     * @notice Creates an arbitrary Wormhole message to be attested by the
     * Wormhole guardians.
     ...
     * - `wormholeMessage` length is >= max(uint16)
     * @param wormholeMessage Arbitrary Wormhole string
    ...
    /**
     * @notice Consumes arbitrary Wormhole messages sent by registered emitters
    ...
    /**
     * @notice Registers foreign emitters (Wormhole contracts) with this contract
